...
and    t.an=2007
and    p.categorie_1='IT'
group by grouping sets ((c.nume, p.categorie_2),(c.nume));
...
and    t.an=2007
and    p.categorie_1='IT'
group by grouping sets ((c.nume, p.categorie_2),(p.categorie_2));
...
and    p.categorie_1='IT'
group by cube (c.nume, p.categorie_2)
having grouping(p.categorie_2) = 1 or grouping(c.nume)=1;
...
select p.denumire, round(sum(v.valoare),1) valoare_totala
from   vanzari_*** v, produse_*** p, timp_*** t
where  v.produs_id=p.id_produs
and    v.timp_id=t.id_timp
and    t.trimestru=1
...
--b
--rownum
select rownum,p.denumire, round(sum(v.valoare),1) valoare_totala
from   vanzari_*** v, produse_*** p, timp_*** t
where  v.produs_id=p.id_produs
and    v.timp_id=t.id_timp
and    t.trimestru=1
...
from (select p.denumire, round(sum(v.valoare),1) valoare_totala
        from   vanzari_*** v, produse_*** p, timp_*** t
        where  v.produs_id=p.id_produs
        and    v.timp_id=t.id_timp
        and    t.trimestru=1
...
--row_number
select ROW_NUMBER() OVER (ORDER BY ROUND(SUM(valoare),1) DESC) row_nr,
